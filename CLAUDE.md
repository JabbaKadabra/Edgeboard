...
Data flow: collectors poll sources on their own asyncio loops → write dicts into one shared `State` object (`edgeboard/state.py`) → the web layer serves `State.snapshot()` via `GET /api/state` and pushes it once per second over SSE at `GET /api/events`. The browser (`edgeboard/static/app.js`) only renders snapshots; the write paths are `POST /api/spotify/{action}`, `POST /api/sessions/{id}/answer` (answers a pending `AskUserQuestion`) and `POST /api/sessions/{id}/send` (a prompt into the session's inbox socket).
